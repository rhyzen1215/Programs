...
<body>
	...
	<?Php
		...
		if (odbc_fetch_row($rs)) {
			...
			$sqlN = "Select * From LeaveApplication where employeeno = '".$EmpNo."' and usedstatus = 'NO'";
			$rsN=odbc_exec($conn,$sqlN) or die("Error in query");
			while (odbc_fetch_row($rsN)) {
			
				$LeaveCnt = odbc_result($rsN,"NoofDays");
				$ToDate = odbc_result($rsN,"DateTo");
				$ToDate = date_create($ToDate);
				$ToDateY = date_format($ToDate,"Y");
				$ToDateM = date_format($ToDate,"m");
				$ToDateM = $ToDateM - 1;
				
				if ($ToDateM == 12) { $ToDateY = $ToDateY - 1; }
				
				$sqlN0 = "Select * From LeaveTable where YearDated = '".$ToDateY."' and MonthDated = '".$ToDateM."' and employeeno = '".$EmpNo."' and leavecode = 'SL'";
				$rsN0=odbc_exec($conn,$sqlN0) or die("Error in query");
				$LEarned = odbc_result($rsN0,"used");
				$LEarned = $LEarned + $LeaveCnt;
				
				$sqlN1 = "Update LeaveTable set used = '".$LEarned."' where YearDated = '".$ToDateY."' and MonthDated = '".$ToDateM."' and employeeno = '".$EmpNo."' and leavecode = 'SL'";
				$rsN1=odbc_exec($conn,$sqlN1) or die("Error in query");
